...
public class GUI_LOGIN extends javax.swing.JFrame {
    ...
    public String getID(String username, String password) {
        ...
        String id = null;
        String query = "SELECT TenTK FROM TaiKhoan WHERE TenTK = ? AND PasswordTK = ?";
        
        try {
            dataService.connectDatabase(); // Kết nối đến cơ sở dữ liệu
            PreparedStatement ps = dataService.getCon().prepareStatement(query);
            ps.setString(1, username);
            ps.setString(2, password);
            
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                id = rs.getString("TenTK");
            }
            
            rs.close();
            ps.close();
            dataService.closeConnection(); // Đóng kết nối
